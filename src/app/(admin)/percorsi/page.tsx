...
export default function PercorsiPage() {
  ...
  const handleUpdateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/percorsi/${editingTemplate.id}`, {
        method: 'PUT',
        body: formDataToSend,
      });
  ...
  const handleDeleteTemplate = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/percorsi/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Percorsi" />
        ...
        <div className="flex flex-col h-full max-h-[90vh]">
          ...
          <div className="flex-shrink-0 p-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {editingTemplate ? 'Modifica Percorso' : 'Aggiungi Nuovo Percorso'}
            </h2>
          </div>
          ...
          <div className="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex justify-end space-x-3">
              ...
              >
                {isSubmitting ? 'Salvataggio...' : (editingTemplate ? 'Aggiorna Percorso' : 'Salva Percorso')}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Grid de plantillas */}
      <ComponentCard title="Modelli di Percorsi">
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <p>Nessun percorso registrato</p>
            <p className="text-sm mt-2">Clicca sull&apos;icona &quot;+&quot; per creare il primo</p>
          </div>
        ) : filteredTemplates.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <p>Nessun percorso trovato con i filtri attuali</p>
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Creato da: {(template as any).creator.firstName} {(template as any).creator.lastName}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
