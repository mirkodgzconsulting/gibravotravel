...
export async function GET(
  ...
  try {
    ...
    const template = await prisma.stop.findUnique({
      where: { id },
      select: {
        id: true,
        title: true,
    ...
    console.error('Error fetching stop template:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    const existingTemplate = await prisma.stop.findUnique({
      where: { id },
    });
    ...
    // Actualizar template
    const updatedTemplate = await prisma.stop.update({
      where: { id },
      data: {
        title,
        textContent,
