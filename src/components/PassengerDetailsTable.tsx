...
const PassengerDetailsTable: React.FC<PassengerDetailsTableProps> = ({ isOpen, onClose }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    const metodoLower = metodoCompraFilter.toLowerCase();
  ...
  const exportToExcel = () => {
    const dataToExport = filteredData.map(item => ({
      ...
      'Neto Servicio': item.neto || 0,
      IATA: item.iata || '',
      PNR: item.pnr || '',
      'Fecha Ida': formatDate(item.servicio && ['volo', 'express', 'polizza'].some(keyword => item.servicio.toLowerCase().includes(keyword)) ? item.andata : null),
      'Fecha Vuelta': formatDate(item.servicio && ['volo', 'express', 'polizza'].some(keyword => item.servicio.toLowerCase().includes(keyword)) ? item.ritorno : null),
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Dettagli Passeggeri');
    XLSX.writeFile(wb, 'detalles_pasajeros.xlsx');
  };

  const handleEstadoChange = async (id: string, newEstado: string, hasFechaPago: boolean) => {
  ...
  const handleSaveFecha = async (id: string, type: 'pago' | 'activacion') => {
    if ((type === 'pago' && !canEditFechaPago) || (type === 'activacion' && !canEditActivacion)) {
      return;
    }
    try {
      await updateDetail(id, {
        [type === 'pago' ? 'fechaPago' : 'fechaActivacion']:
          type === 'pago' ? (tempFechaPago || null) : (tempFechaActivacion || null),
      });
      if (type === 'pago') {
        setEditingFechaPagoId(null);
        setTempFechaPago('');
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full h-full max-w-none max-h-none flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 className="text-sm font-medium text-gray-900 dark:text-white">
              Dettagli per Passeggero e Servizio
            </h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Informazioni consolidate e modificabili per passeggero e servizio
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3">
            {[
              {
                ...
                value: fechaDesde,
                setter: setFechaDesde,
                container: 'bg-slate-50 dark:bg-slate-900/20',
            ...
            ].map(({ label, value, setter, container, labelClass, borderClass, ringClass }) => (
              <div key={label} className={`${container} p-2 rounded-lg`}>
                <label className={`block text-xs font-medium ${labelClass} mb-1`}>{label}</label>
                <input
                  type="date"
                  value={value}
                        ...
                        {item.servicio && ['volo', 'express', 'polizza'].some(keyword => item.servicio.toLowerCase().includes(keyword))
                          ...
                          <TableCell className="py-2 px-3 text-xs">
                            {canEditEstado && editingEstadoId === item.id ? (
                              <select
                                value={item.estado}
                                onChange={(e) => handleEstadoChange(item.id, e.target.value, !!item.fechaPago)}
                          ...
                          <TableCell className="py-2 px-3 text-xs">
                            {canEditFechaPago && editingFechaPagoId === item.id ? (
                              <div className="flex items-center space-x-1">
                                ...
                                />
                                <button
                                  onClick={() => handleSaveFecha(item.id, 'pago')}
                                  disabled={isUpdating}
                          ...
                          <TableCell className="py-2 px-3 text-xs">
                            {canEditActivacion && editingFechaActivacionId === item.id ? (
                              <div className="flex items-center space-x-1">
                                ...
                                />
                                <button
                                  onClick={() => handleSaveFecha(item.id, 'activacion')}
                                  disabled={isUpdating}
                                  className="px-2 py-1 text-xs bg-green-500 text-white rounded hover:bg-green-600 disabled:opacity-50"
                                  title="Salva"
                                >
                              ...
                              <button
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  setTempFechaActivacion(item.fechaActivacion ? item.fechaActivacion.split('T')[0] : '');
                          ...
                          <TableCell className="py-2 px-3 text-xs">
                            {canEditNotas && editingNotasId === item.id ? (
                              ...
                              <button
                                ...
                                className="text-xs px-2 py-1 rounded border border-gray-300 hover:bg-gray-100 text-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700 w-full text-left min-h-[32px] flex items-center"
                                title="Clicca per modificare"
                              >
