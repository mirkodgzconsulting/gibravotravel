...
export async function GET(
  ...
  try {
    ...
    const template = await prisma.info.findUnique({
      where: { id },
      select: {
        id: true,
        title: true,
    ...
    console.error('Error fetching info template:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    const existingTemplate = await prisma.info.findUnique({
      where: { id },
    });
    ...
    if (existingTemplate.createdBy !== userId) {
      return NextResponse.json(
        { error: 'No tienes permisos para editar este template' },
        { status: 403 }
      );
    }
    ...
    // Actualizar template
    const updatedTemplate = await prisma.info.update({
      where: { id },
      data: {
        title,
        textContent,
    ...
    console.error('Error updating info template:', error);
    return NextResponse.json(
      { 
        error: 'Error interno del servidor',
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error deleting info template:', error);
    return NextResponse.json(
      { 
        error: 'Error interno del servidor',
        details: error instanceof Error ? error.message : 'Unknown error'
