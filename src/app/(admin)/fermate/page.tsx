...
export default function FermatePage() {
  ...
  const [editingTemplate, setEditingTemplate] = useState<StopTemplate | null>(null);
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      console.log('🔍 [FERMATE FRONTEND] Fetching templates...');
      setLoading(true);
      setError(null);
      const response = await fetch('/api/stop');
      console.log('📊 [FERMATE FRONTEND] Response status:', response.status);
      console.log('📊 [FERMATE FRONTEND] Response ok:', response.ok);

      if (response.ok) {
        const data = await response.json();
        console.log('✅ [FERMATE FRONTEND] Templates loaded:', data.templates?.length || 0);
        ...
        try {
          ...
          console.error('❌ [FERMATE FRONTEND] Error parsing response:', parseError);
          errorData = { error: 'Error de respuesta del servidor' };
        }
        console.error('❌ [FERMATE FRONTEND] Error from API:', errorData);
        setError(`Error al cargar plantillas: ${errorData.error || 'Error desconocido'}`);
      }
    } catch (error) {
      console.error('❌ [FERMATE FRONTEND] Network error:', error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('🚀 [FERMATE FRONTEND] Form submission started');
    console.log('👤 [FERMATE FRONTEND] User:', user?.id);
    console.log('📝 [FERMATE FRONTEND] Form data:', formData);

    if (!user) {
      console.log('❌ [FERMATE FRONTEND] No user, aborting');
      return;
    }
    ...
    try {
      ...
      console.log('📤 [FERMATE FRONTEND] Sending request to /api/stop');
      const response = await fetch('/api/stop', {
        method: 'POST',
        body: formDataToSend,
      });
      ...
      if (response.ok) {
        ...
        console.error('❌ [FERMATE FRONTEND] API Error:', errorData);
  ...
  const handleEditTemplate = (template: StopTemplate) => {
  ...
  const handleUpdateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/stop/${editingTemplate.id}`, {
        method: 'PUT',
        body: formDataToSend,
      });
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Fermate" />
      ...
      {/* Grid de plantillas */}
      <ComponentCard title="Modelli di Fermate">
        {loading ? (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-2 mb-3">
                    ...
                    {(template as any).coverImage && (template as any).coverImage !== null && (
                      <div className="text-sm">
