...
export default function UserProfilePage() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: 'No se pudo cargar el perfil (API)' });
      }
    } catch (error) {
      console.error('Error fetching user profile:', error);
      setMessage({ type: 'error', text: 'Error de red al cargar el perfil' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Notificar al header/dropdown que el perfil cambió para actualizar avatar al instante
        try {
  ...
  // Fallback: si no hay userProfile, usar datos básicos de Clerk para mostrar algo
  const fallbackFromClerk: UserProfile | null = clerkUser
    ? {
        id: clerkUser.id,
  ...
  const displayUser = userProfile || fallbackFromClerk;
                ...
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      getRoleBadgeColor(displayUser.role) === 'error' ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-200' :
                      getRoleBadgeColor(displayUser.role) === 'warning' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-200' :
                      getRoleBadgeColor(displayUser.role) === 'success' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-200' :
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-200">
                      {displayUser.isActive ? 'Activo' : 'Inactivo'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900 dark:text-white">
                    {displayUser.phoneNumber || 'No especificado'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 dark:text-white">
                    {new Date(displayUser.updatedAt).toLocaleDateString('es-ES', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
